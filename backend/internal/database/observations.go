...
func GetObservation(id int) (Observation, error) {
	...
	obs := Observation{}
	sql, args := sb.Build()
	err := dbpool.QueryRow(context.Background(), sql, args...).Scan(&obs.ID, &obs.UserID, &obs.Name, &obs.Address, &obs.Styles, &obs.Type, &obs.Condition, &obs.Lng, &obs.Lat, &obs.Year, &obs.ImgCount)
	if err != nil {
